...
app.secret_key = os.getenv(
    'SECRET_KEY', uuid.uuid4().hex)  # .envファイルからシークレットキーを取得
...
def index():
    ...
    if user_id is None:
        # ←セッションがない場合login_page関数からログイン画面に戻る
...
def login_process():
    ...
    if email == '' or password == '':
        ...
        # find_userメソッドへ、emailを引数で渡し、SELECT文でユーザー情報を辞書型で全て取得
...
def home_page():
    ...
    if user_id is None:
        ...
        # room_id,room_name,owner_id一覧情報を昇順で取得
